    ...
    >
      ...
      <div className="relative max-w-5xl mx-auto px-4">
        ...
        <nav className={clsx(
          ...
          isScrolling ? "md:px-20 px-10 fixed top-0 left-0 right-0 bg-white/95 shadow-lg z-50 text-gray-800" : "relative"
        )}>
          <div className="flex items-center justify-between w-full">
            ...
            <div className="hidden md:flex gap-4">
              <button className={clsx(
                "px-4 py-2",
                isScrolling ? "text-gray-800 hover:text-blue-500" : "text-white hover:text-blue-200"
              )}>
                Log in
              </button>
            ...
            <div className="md:hidden" onClick={() => setOpenMobileMenu(!openMobileMenu)}>
              {openMobileMenu ? (
                <X size={25} color={isScrolling ? "black" : "white"} /> // Color changes based on scroll
              ) : (
                <Menu size={25} color={isScrolling ? "black" : "white"} className="cursor-pointer" /> // Color changes based on scroll
              )}
            </div>
          </div>
        ...
        {/* Hero Content */}
        <div className="min-h-[calc(600px-5rem)] flex flex-col items-center justify-center text-center text-white">
          ...
          <div className="flex flex-row gap-2 w-full max-w-3xl bg-white p-2 rounded-lg items-center md:gap-4">
            ...
            <div className="relative min-w-[150px] sm:min-w-[200px] md:min-w-[250px]">
              <div className="relative flex items-center">
                ...
                <ChevronDown
                  className={`absolute right-2 sm:right-3 top-1/2 transform -translate-y-1/2 text-gray-600 transition-transform duration-200 cursor-pointer ${isCategoryOpen ? "rotate-180" : "rotate-0"}`}
                  size={16}
                />
              </div>
            </div>
            ...
            <div className="flex-1 relative min-w-0">
              <div className="relative flex items-center">
                ...
                <ChevronDown
                  className={`absolute right-2 sm:right-3 top-1/2 transform -translate-y-1/2 text-gray-600 transition-transform duration-200 cursor-pointer ${isLocationOpen ? "rotate-180" : "rotate-0"}`}
                  size={16}
                />
              </div>
            </div>
