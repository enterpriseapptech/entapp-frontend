...
export default function SearchPage() {
  ...
  // Client-side location filter for catering (catering.location is a string[])
  // The backend may not filter correctly, so we ensure only services in the
  // selected state (locationId) are shown.
  const filteredCateringItems = (cateringData?.data ?? []).filter(
    (c) => !locationId || c.location.includes(locationId)
  );
  const filteredCateringCount = filteredCateringItems.length;

  // Event centers location field is a string — backend handles the filter.
  const totalResults =
    (eventCentersData?.count || 0) + filteredCateringCount;
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!isLoading && (
          <div className="space-y-12">
            ...
            {/* No event centers in this area (only shown in "all" mode when catering has results) */}
            {category === "all" &&
              eventCentersData &&
              eventCentersData.count === 0 &&
              filteredCateringCount > 0 && (
                <div className="flex items-center gap-3 px-4 py-3 bg-blue-50 border border-blue-200 rounded-lg text-blue-700 text-sm">
                  ...
                  <span>No event centers found in <strong>{state}</strong>. Showing catering services only.</span>
                </div>
              )}

            {/* Catering Results */}
            {(category === "all" || category === "CATERING") && filteredCateringCount > 0 && (
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-gray-900">
                    Catering Services
                  ...
                  <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-semibold">
                    {filteredCateringCount} found
                  </span>
                </div>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredCateringItems.map((catering) => (
                    <Link 
                      key={catering.id} 
